...
def get_params():
    ...
    """
    ret = requests.get("https://m.touker.com/account/login/index.htm?source=stock_trade_h5&referrer=https://m.touker.com/trading/trade/position#/")
    s = ret.text
    cookies = ret.cookies
    cookies_dict =  requests.utils.dict_from_cookiejar(cookies)
    try:
        hbtoken = re.search('id="_hbtoken_" value="(.*?)"',s).group(1)
        key = re.search('encryptor.setPublicKey\("(.*?)"',s).group(1)
